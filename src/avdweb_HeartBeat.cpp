 ...
 * Version 8-9-2017 low number overrides high number, changed because of new
...
bool SimpleSoftPWM::pwm(byte value) // 0 ... 255, only for LEDs
{
  ...
  if ((now_us - start_us) >= periodTime_us)
    start_us = now_us;
  return ((now_us - start_us) < value * 78); // 19890/255=78
}

HeartBeat::HeartBeat(byte LEDpin) : LEDpin(LEDpin) { pinMode(LEDpin, OUTPUT); }
...
void HeartBeat::heartBeat() {
  ...
  if ((now_ms - last_ms) < stepInterval_ms)
    return;
  last_ms = now_ms;
  if (Uled >= UledMax)
    i = -stepValue;
...
void HeartBeat::blinkLED(int _blinkCounts) {
  if (blinkCounts == 0)
    blinkCounts = _blinkCounts;
  else if (_blinkCounts < blinkCounts)
    blinkCounts = _blinkCounts; // low number overrides high number
}
...
void HeartBeat::blink() {
  ...
  if (blinkOnDelay.elapsed()) {
    if (i++ >= blinkCounts) {
      i = 0;
      blinkCounts = 0; // reset, blinkCount is set again in blinkCount() which
                       // has to be called in the loop repeatedly
    }
