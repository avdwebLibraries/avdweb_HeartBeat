...
void test_blinkLED() {
  ...
  if (millis() > 3000 && millis() < 12000)
    heartBeat.blinkLED(3); // blink LED 3x
  if (millis() > 8000 && millis() < 10000)
    heartBeat.blinkLED(2); // blink LED 2x, because 2<3
  if (millis() > 6000 && millis() < 12000)
    heartBeat.blinkLED(4); // ignored because 2<4
  // heartBeat between blinkLED
  if (millis() > 19000 && millis() < 24000)
    heartBeat.blinkLED(1);
}
